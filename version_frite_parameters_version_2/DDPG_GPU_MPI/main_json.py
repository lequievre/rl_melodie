...
def main():
	...
	if args.mode == 'test':
	...
	elif args.mode == 'test_ik_real_move':
		input("hit return to reach HOME position and orientation with joints positions with real robot")
		
		env.go_to_position_with_orientation([0.554, 0.000, 0.521],[1.0, 0.0, 0.0, 0.0])
		...
		for i in range(5):
			xmin += delta_x
			env.go_to_position_with_orientation([xmin, 0.000, 0.521],[1.0, 0.0, 0.0, 0.0])
			
		
		input("hit to finish !")
		
	
	elif args.mode == 'test_ik_real':
		
		# HOME POSITION
		# '{pose: {position: {x: 0.554, y: 0.000, z: 0.521} , orientation: {x: 1.0, y: 0.0, z: 0.0, w: 0.0} }  }'
		...
		# position vibrante : '{pose: {position: {x: 0.37987376261171857, y: 0.1354714113344091, z: 0.3217492984212611} , orientation: {x: 0.9862637961417702, y: -0.010092002042582158, z: 0.16391032039999828, w: -0.017620129946211333} }  }'

		input("hit return to reach VIBRATE position and orientation with joints positions with real robot")
		
		#env.go_to_position_with_orientation([0.37987376261171857, 0.1354714113344091, 0.3217492984212611],[0.9862637961417702, -0.010092002042582158, 0.16391032039999828, -0.017620129946211333])
		
		
		#env.go_to_position_with_orientation([0.379, 0.135, 0.321],[0.986, -0.010, 0.163, -0.017])
		env.go_to_position_with_orientation([0.379, 0.135, 0.321],[1.0, 0.0, 0.0, 0.0])
		
		#time.sleep(5)
		
		input("hit return to finish !")
			
	elif args.mode == 'test_orientation':
		
		env.reset()
		
		# python main_json.py --mode test_orientation --config_file 'trains_tests/version_json_frite_parameters/test_orientation/config.json' --gui True
		# don't forget to set 'is_ro_version' flag to 'false' into config file
		
		"""env.debug_gui.reset()
		
		env.env_pybullet.reset()
		...
		env.create_anchor_panda()
		#p.stepSimulation()
		
		"""
		
		input("hit return to initial position")
		
		env.go_to_position_orientation_simulated([0.554, 0.000, 0.521],[1.0, 0.0, 0.0, 0.0])
		
		input("hit return to print position and orientation")
		
		env.print_current_position_and_orientation()
		"""
		input("hit return to initial orientation")
		
		env.set_initial_gripper_orientation(0.5, 0.5, 0.0)
		
		env.set_panda_to_initial_gripper_orientation()
		"""
		input("hit return to go to vibrate position")
		
		env.go_to_position_orientation_simulated([0.379, 0.135, 0.321],[0.986, -0.010, 0.163, -0.017])
