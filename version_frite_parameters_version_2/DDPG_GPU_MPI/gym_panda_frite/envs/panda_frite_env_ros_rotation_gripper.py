...
class PandaFriteEnvROSRotationGripper(gym.Env):
	...
	def __init__(self, database = None, json_decoder = None, env_pybullet = None, gui = None):
		...
		if is_ros_version:
			...
			print("panda lower limits={}, upper_limits={}, ranges={}, initial poses={}".format(self.panda_list_lower_limits, self.panda_list_upper_limits, self.panda_list_joint_ranges, self.panda_list_initial_poses))
		# show sliders
	...
	def print_current_position_and_orientation(self):
		cur_state = p.getLinkState(self.panda_id, self.panda_end_eff_idx)
		cur_pos = np.array(cur_state[0])
		cur_orien_quaternion = np.array(cur_state[1])
		cur_orien_euler = p.getEulerFromQuaternion(cur_orien_quaternion)
		
		print("Current gripper pos={}".format(cur_pos))
		print("Current gripper orien euler={}".format(cur_orien_euler))
		print("Current gripper orien quaternion={}".format(cur_orien_quaternion))
		
	
	
	def go_to_position_orientation_simulated(self, a_position, an_orientation):
		
		
		new_pos = a_position
		new_orien = an_orientation
		jointsPositions = p.calculateInverseKinematics(self.panda_id, self.panda_end_eff_idx, new_pos, new_orien, maxNumIterations=1000, residualThreshold=0.001)[0:7]
		
		#jointsPositions = self.accurateCalculateInverseKinematics(self.panda_id, self.panda_end_eff_idx, a_position, an_orientation, 0.001, 1000)
		
		
		for i in range(len(jointsPositions)):
			p.setJointMotorControl2(self.panda_id, i, p.POSITION_CONTROL, jointsPositions[i],force=self.joint_motor_control_force * 240.)
	...
	def go_to_home_position_with_joint_position(self, positions):
		# [ 0.0, 0.0, 0.0,-math.pi/2.,0.0,math.pi/2,math.pi/4.]
		
		home_position =np.array([0.554, 0.000, 0.521])
		#input("**ENV** Hit return : go_to_home_position_with_joint_position = {}".format(positions))
		
		self.publish_joint_position(positions)
		...
		while True:
			...
			if np.linalg.norm(current_tip_position - home_position, axis=-1) <= 0.005:
				print("Exit with precision O.005 !")
				break
	...
	def accurateCalculateInverseKinematics(self, pandaId, endEffectorId, targetPos, targetQuat, threshold, maxIter):
		closeEnough = False
		iter = 0
		dist2 = None
		while (not closeEnough and iter < maxIter):
			# ajouter min et max ??
			# cf : https://www.programcreek.com/python/?code=SudeepDasari%2Fvisual_foresight%2Fvisual_foresight-master%2Fvisual_mpc%2Fenvs%2Frobot_envs%2Fwidowx%2Fwidowx_controller.py
			jointPoses = list(p.calculateInverseKinematics(pandaId, endEffectorId, targetPos, targetQuat, self.panda_list_lower_limits, self.panda_list_upper_limits)[0:7])
			for i in range(len(jointPoses)):
				jointPoses[i] = max(min(jointPoses[i], self.panda_list_upper_limits[i]), self.panda_list_lower_limits[i])
				p.resetJointState(pandaId, i, jointPoses[i])
			ls = p.getLinkState(pandaId, endEffectorId, computeForwardKinematics=1)
			newPos = ls[0]
	...
	def go_to_position_with_orientation(self, a_position, an_orientation):
		print("**ENV** go_to_position_with_orientation -> a_position = {}, an_orientation = {}".format(a_position,an_orientation))
		...
		for i in range(len(jointsPositions)):
			p.resetJointState(self.panda_id, i, jointsPositions[i])
			
		#jointsPositions = self.accurateCalculateInverseKinematics(self.panda_id, self.panda_end_eff_idx, a_position, an_orientation, 0.005, 100)
		
		print("**ENV** go_to_position_with_orientation -> jointsPositions = {}".format(jointsPositions))
		
		#input("**ENV** Hit return : go_to_position_with_orientation with joints positions = {}".format(jointsPositions))
		
		self.publish_joint_position(jointsPositions)
		...
		while True:
			...
			if np.linalg.norm(current_tip_position - a_position, axis=-1) <= 0.005:
				print("Exit with precision 0.005!")
				break
		...
		"""
		self.publish_position_with_orientation(a_position,an_orientation)
		...
		"""
	"""
	def go_to_position_by_joint_position(self, a_position, an_orientation):
		new_pos = a_position
		new_orien = an_orientation
		jointsPositions = p.calculateInverseKinematics(self.panda_id, self.panda_end_eff_idx, new_pos, new_orien)[0:7]
		...
		while True:
			...
			if np.linalg.norm(current_tip_position - a_position, axis=-1) <= 0.01:
				print("Exit with precision !")
				break
	...
	def go_to_home_position(self):
		if self.database.type_db_load == 2 and self.fix_initial_gripper_orientation:
			# get initial orientation from sample_goal
			# End Eff home position => x=0.554, y=0.000, z=0.521
			#new_orien_quaternion = tf.transformations.quaternion_from_euler(self.initial_gripper_orientation[0],self.initial_gripper_orientation[1],self.initial_gripper_orientation[2])
			cur_orien_euler = p.getEulerFromQuaternion([1.0, 0.0, 0.0, 0.0])
			print("cur orien euler={}, orien_goal={}".format(cur_orien_euler,self.initial_gripper_orientation))
			new_orien_euler = [0.0, 0.0, 0.0]
			new_orien_euler[0] = cur_orien_euler[0] + self.initial_gripper_orientation[0]
			new_orien_euler[1] = cur_orien_euler[1] + self.initial_gripper_orientation[1]
			new_orien_euler[2] = cur_orien_euler[2] + self.initial_gripper_orientation[2]
			
			
			new_orien_quaternion = p.getQuaternionFromEuler(new_orien_euler)
			
			#new_orien_quaternion = p.getQuaternionFromEuler(self.initial_gripper_orientation)
			print("FIXED INITIAL GRIPPER ORIENTATION !")
			print("pos={}, orien quater={}, orien euler={}".format(np.array([0.554, 0.000, 0.521]),new_orien_quaternion,new_orien_euler))
			
			
			initial_orientation_quaternion = [1.0, 0.0, 0.0, 0.0]
			
			#self.go_to_home_position_with_joint_position([ 0.0, 0.0, 0.0,-math.pi/2.,0.0,math.pi/2,math.pi/4.])
			
			
			#time.sleep(5.0)
			
			self.go_to_position_with_orientation(np.array([0.554, 0.000, 0.521]),new_orien_quaternion)
			
			time.sleep(5.0)
			
		else:
			print("NO FIXED INITIAL GRIPPER ORIENTATION !")
			# End Eff home position => x=0.554, y=0.000, z=0.521
			self.go_to_position(np.array([0.554, 0.000, 0.521]))
			# End Eff home position for tablet => x=0.554, y=0.000, z=0.581
			# self.go_to_position(np.array([0.554, 0.000, 0.571]))
	
	
	def publish_position_with_orientation(self, command, an_orientation):
	...
	def convertToJointStateMsg(self,positions,velocities):
		msg = JointState()
		#msg.header = Header()
		#msg.name = ['joint1','joint2','joint3','joint4','joint5','joint6','joint7']
		msg.position = [positions[0],positions[1],positions[2],positions[3],positions[4],positions[5],positions[6]]
		msg.velocity = [velocities[0],velocities[1],velocities[2],velocities[3],velocities[4],velocities[5],velocities[6]]
		#msg.effort = []
		return msg
	
	"""
	def go_to_joint_position(self, positions):
		self.publish_joint_position(positions)
	"""	
		
	def publish_joint_position(self, positions):
		position_msg = self.convertToJointStateMsg(positions,[0.5,0.5,0.5,0.5,0.5,0.5,0.5])
	...
	def init_ros(self):
		...
		self.panda_list_lower_limits, self.panda_list_upper_limits, self.panda_list_joint_ranges, self.panda_list_initial_poses = self.get_panda_joint_ranges()
		print("#init_ros -> panda lower limits={}, upper_limits={}, ranges={}, initial poses={}".format(self.panda_list_lower_limits, self.panda_list_upper_limits, self.panda_list_joint_ranges, self.panda_list_initial_poses))
		...
		rospy.Subscriber('/joint_position_trapezoid_controller_pi/current_observation_orientation', Float64MultiArray, self.observation_callback,
						 queue_size=10)
		...
		self.publisher_joint_position = rospy.Publisher('/joint_position_trapezoid_controller_pi/command', JointState, queue_size=10)
	...
	def sample_goal(self):
		if self.database.type_db_load == 2:
			...
			print("goal_with_frite_parameters = {}".format(goal_with_frite_parameters[0]))
	...
	def set_gym_spaces_ros(self):
		...
		print("low_z_up={}".format(low_z_up))
		print("pos space={}".format(self.pos_space))
	...
	def set_action_ros(self, action):
		...
		cur_orien_quaternion = p.getQuaternionFromEuler(tip_orientation)
		
		if self.database.type_db_load == 2:
			# if type is frite parameters
			if self.fix_initial_gripper_orientation:
				new_orien_quaternion = cur_orien_quaternion
			else:
	...
	def step_ros(self, action):
		...
		mean_reward = sum_d/nb_mesh_to_follow
		
		info = {
			'is_success': self.is_success(mean_reward),
			'max_distance_error' : mean_reward,
		}
		
		reward = -mean_reward
		if (mean_reward > self.distance_threshold):
			done = False
